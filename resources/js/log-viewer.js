...
class LogViewer {
    ...
    async start() {
        ...
        containers.forEach((container, index) => {
            ...
            this.streamContainer(container, color, isMulti);
        });
    }
    ...
    async discoverContainers() {
        ...
        if (type === 'all') {
            const data = await this.fetchJson('/api/stacks', headers);
            const containers = [];
            for (const s of data.stacks ?? []) {
                const detail = await this.fetchJson(`/api/stacks/${encodeURIComponent(s.name)}`, headers);
        ...
        if (type === 'stack') {
            const detail = await this.fetchJson(`/api/stacks/${encodeURIComponent(stack)}`, headers);
            const containers = [];
            for (const svc of detail.services ?? []) {
                for (const c of svc.containers ?? []) {
                    if (c.state === 'running') {
            ...
            try {
                ...
                while (true) {
                    ...
                    for (const raw of complete.split('\n')) {
                        ...
                        // Strip trailing CR if present (Windows-origin logs)
                        if (text.endsWith('\r')) {
                            text = text.slice(0, -1);
                        }
...
function startViewer(secret) {
    ...
    // Clear previous terminal DOM content before re-opening
    container.innerHTML = '';

    const scope = window.__LOG_SCOPE ?? { type: 'all' };
    activeViewer = new LogViewer(scope, secret);
    activeViewer.init(container);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Restore secret from session so page reloads don't require re-auth
    const stored = sessionStorage.getItem('log_viewer_secret');
    if (stored) {
        hideAuthOverlay();
    ...
    authForm?.addEventListener('submit', (e) => {
        ...
        sessionStorage.setItem('log_viewer_secret', secret);
        hideAuthOverlay();
        disconnectBtn?.classList.remove('hidden');
        startViewer(secret);
    });
    ...
    disconnectBtn?.addEventListener('click', () => {
        activeViewer?.destroy();
        activeViewer = null;
        sessionStorage.removeItem('log_viewer_secret');
