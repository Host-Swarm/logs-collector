...
<body class="bg-[#0d1117] text-[#c9d1d9] h-full flex flex-col overflow-hidden">
    ...
    <header class="bg-[#161b22] border-b border-[#30363d] px-4 py-2 flex items-center gap-3 shrink-0">
        ...
        <nav class="flex items-center gap-1.5 text-sm min-w-0 flex-1">
            <a href="/api/logs" class="text-[#58a6ff] hover:underline shrink-0">All Logs</a>

            @if (!empty($scope['stack']))
                <span class="text-[#484f58]">/</span>
                @if (!empty($scope['service']) || !empty($scope['container_id']))
                    <a href="/api/logs/{{ rawurlencode($scope['stack']) }}" class="text-[#58a6ff] hover:underline truncate max-w-[160px]" title="{{ $scope['stack'] }}">{{ $scope['stack'] }}</a>
                @else
            ...
            @if (!empty($scope['service']))
                ...
                @if (!empty($scope['container_id']))
                    <a href="/api/logs/{{ rawurlencode($scope['stack']) }}/{{ rawurlencode($scope['service']) }}" class="text-[#58a6ff] hover:underline truncate max-w-[160px]" title="{{ $scope['service'] }}">{{ $scope['service'] }}</a>
                @else
